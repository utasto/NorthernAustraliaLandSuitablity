...
readSuitFramework <- function(rootDir, suitFile, sheet, classFile ){
 ...
 suits <- merge(x = limClasses, y = subClasses[-drops] , by.x="LIM_VALUE", by.y = "LIM_VALUE", all.x=T, all.y=T)
...
calcSubClasses <- function(rootDir, catchmentName, lim, useList, suitFramework, limRastersList, limInfo){
  ...
  limVals1 <- readRDS(paste0(rootDir, '/Attributes/', limMappping1[1,4], '/', catchmentName, '/Chunks/r_', k, '.rds'))
  cellids <-  limVals1[,1]
  warr1 <- limVals1[,-1]
