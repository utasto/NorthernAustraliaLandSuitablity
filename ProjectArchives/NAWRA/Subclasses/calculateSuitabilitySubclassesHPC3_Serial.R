...
##### debug on TernSoils VM   ####
    ...
    source('//OSM-09-CDC.it.csiro.au/OSM_CBR_LW_SLGA_work/Projects/NAWRA/Production/Scripts/Utils/process_Lims_V2.R')
...
#######################################
  ...
  source('/OSM/CBR/LW_SLGA/work/Projects/NAWRA/Production/Scripts/Utils/process_Lims_V2.R')
...
limInfo <- suitFramework[ suitFramework$LIM_Name==lim,]
 ...
 for(k in 1:length(bs$row)){
    processSubClassesParallel(rootDir, catchmentName, lim, useList, suitFramework, limRastersList, bs, limInfo, k)
 }
