...
makerasterFromTreePredictionChunks <- function(ProcessingDir = NULL, templateR = NULL, att = NULL, model = NULL , islog=T){
  ...
  for (i in 1:length(fls)){
    ...
    if(islog == TRUE){
      ...
      b[blockVals$inds] <- exp(blockVals$modalVal)
      c[blockVals$inds] <- exp(blockVals$ci)} else {
...
if(machineName=='TERNSOIL2'){
  ...
  k=as.numeric(args[1])
  att = 'logA2b2'
  modelFileName = 'logA.mod.2b2.rds'
  chunkLines = 20
...
####look at raster
r <- raster('Q:/3_Land_suitability/0_Working/Uta/Roper/Maps/logESP3/logESP3.tif')
